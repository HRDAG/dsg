...
class TestPerformanceCharacteristics:
    ...
    def test_sync_performance_with_larger_datasets(self, dsg_repository_factory):
        ...
        with EndToEndWorkflowContext("performance-test") as context:
            ...
            # Create multiple data files with realistic sizes
            for i in range(10):
                data_file = context.local_repo / 'input' / f'large_dataset_{i}.csv'
                data_file.parent.mkdir(parents=True, exist_ok=True)
                
                # Create larger CSV content
                lines = ['id,value,category,timestamp']
                for j in range(100):  # 100 rows per file
                    lines.append(f'{i*100+j},{j*10},category_{j%5},2025-01-{(j%30)+1:02d}')
                
                data_file.write_text('\n'.join(lines))
                performance_files.append(data_file)
            
            # Create multiple code files
            for i in range(5):
                code_file = context.local_repo / 'src' / f'analysis_module_{i}.py'
                code_file.parent.mkdir(parents=True, exist_ok=True)
                
                # Create realistic Python code content
                code_content = f'''#!/usr/bin/env python3
"""
Analysis module {i} for performance testing.
"""
...
def process_data_{i}(df: pd.DataFrame) -> pd.DataFrame:
    """Process data for module {i}."""
    # Realistic data processing code
    df_filtered = df[df['category'] == 'category_{i%5}']
    df_aggregated = df_filtered.groupby('category').agg({{
        'value': ['mean', 'std', 'count'],
        'id': 'nunique'
    }})
    return df_aggregated

def analyze_trends_{i}(data: List[Dict[str, Any]]) -> Dict[str, float]:
    """Analyze trends for module {i}."""
    # More realistic analysis code
...
# Additional functions and classes for module {i}
...
'''
                ...
                performance_files.append(code_file)
            
            # Create documentation files
            for i in range(3):
                doc_file = context.local_repo / 'docs' / f'analysis_guide_{i}.md'
                doc_file.parent.mkdir(parents=True, exist_ok=True)
                
                doc_content = f'''# Analysis Guide {i}

## Overview
This guide covers analysis procedures for module {i}.

## Data Requirements
- Input data should be in CSV format
- Minimum 100 rows required for statistical significance
- Categories should be predefined

## Methodology
1. Data preprocessing and cleaning
2. Statistical analysis using standard methods
3. Trend analysis and forecasting
4. Result validation and quality checks

## Code Examples

```python
# Example usage for module {i}
import analysis_module_{i} as am{i}

# Load and process data
df = pd.read_csv('large_dataset_{i}.csv')
processed_df = am{i}.process_data_{i}(df)

# Analyze trends
trends = am{i}.analyze_trends_{i}(processed_df.to_dict('records'))
print(f"Trend analysis results: {{trends}}")
```

## Performance Notes
- Expected processing time: ~{i+1} seconds per 1000 rows
- Memory usage: ~{(i+1)*10}MB for typical datasets
- Optimal batch size: {(i+1)*100} rows

## Troubleshooting
Common issues and solutions for module {i} operations.
'''
                doc_file.write_text(doc_content)
                performance_files.append(doc_file)
            
            # Measure initial sync performance
            print(f"Starting performance test with {len(performance_files)} files")
            start_time = time.time()
            
            initial_sync_plan = {
                'upload_files': [str(f.relative_to(context.local_repo)) for f in performance_files],
                'download_files': [],
                'delete_local': [],
                'delete_remote': []
            }
            ...
            print(f"✓ Initial sync completed in {initial_sync_time:.2f} seconds")
            ...
            # Modify existing files
            for i in range(3):
                file_to_modify = context.local_repo / 'src' / f'analysis_module_{i}.py'
                current_content = file_to_modify.read_text()
                modified_content = f"{current_content}\n\n# Performance test modification {i}\ndef perf_test_function_{i}():\n    pass\n"
                file_to_modify.write_text(modified_content)
                modified_files.append(file_to_modify)
            
            # Add new files
            new_files = []
            for i in range(2):
                new_file = context.local_repo / 'output' / f'performance_results_{i}.csv'
                new_file.parent.mkdir(parents=True, exist_ok=True)
                new_file.write_text(f'test_id,duration,status\nperf_{i},{initial_sync_time:.3f},completed\n')
                new_files.append(new_file)
            ...
            incremental_sync_plan = {
                'upload_files': [str(f.relative_to(context.local_repo)) for f in modified_files + new_files],
                'download_files': [],
                'delete_local': [],
                'delete_remote': []
            }
            ...
            with context.create_transaction() as tx:
                tx.sync_files(incremental_sync_plan)
            
            incremental_sync_time = time.time() - start_time
            
            print(f"✓ Incremental sync completed in {incremental_sync_time:.2f} seconds")
            
            # Performance assertions
            assert initial_sync_time < 60.0, f"Initial sync should complete within 60s, took {initial_sync_time:.2f}s"
            assert incremental_sync_time < 30.0, f"Incremental sync should complete within 30s, took {incremental_sync_time:.2f}s"
            assert incremental_sync_time < initial_sync_time, "Incremental sync should be faster than initial sync"
            
            # Verify all files were synced correctly
            if Path(context.zfs_mount).exists():
                zfs_files = list(Path(context.zfs_mount).rglob("*.py"))
                assert len(zfs_files) >= 5, "All Python files should be in ZFS"
                
                zfs_csv_files = list(Path(context.zfs_mount).rglob("*.csv"))
                assert len(zfs_csv_files) >= 10, "All CSV files should be in ZFS"
            
            print(f"✓ Performance test passed - Initial: {initial_sync_time:.2f}s, Incremental: {incremental_sync_time:.2f}s")