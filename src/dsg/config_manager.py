...
def create_backend(config: Config):
    ...
    For SSH transport, detects if target is effectively localhost and
    uses filesystem operations for performance. Otherwise uses network SSH.
    
    Args:
        config: Complete DSG configuration
        
    Returns:
        Backend instance (LocalhostBackend or SSHBackend)
